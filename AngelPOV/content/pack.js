...
export const pack = {
  ...
  links: {
    ...
    // Generated by tools/build_notes.mjs. Relative on purpose: it sits beside index.html
    // in both the canonical tree and the deploy tree, unlike the .md links above.
    notes: './notes.html',
  },
  ...
  computeEnding(s) {
    ...
    if (f.road === 'east' && q.crown_ottoman >= 12 && f.turk !== 'war') {
      return {
        ...
        evidence: {
          source: 'COUNTERFACTUAL \u2014 Melvin-Koushki, Hellebore (2021), 76\u201378',
          text:
            'None of this happened, and the note matters more here than anywhere else in the ' +
            'game. What is real: the capitulations Elizabeth agreed with the Porte, the Levant ' +
            'Company running English ships to Constantinople twice a year, an English factory ' +
            'in the Ottoman capital, and a destitute Dee refused by two Christian courts. What ' +
            'is real about the destination: Murad III (d. 1595) kept a detailed record of his ' +
            'visions much like Dee\u2019s own, and Ottoman sovereigns from Bayezid II onward ' +
            'cultivated a millennial imperial ideology in which lettrism, astrology and ' +
            'talismanry were instruments of statecraft rather than suspect arts. What is ' +
            'invented is the voyage. Melvin-Koushki proposes it as a serious counterfactual on ' +
            'the grounds that the Ottoman court was measurably more hospitable to Dee\u2019s exact ' +
            'combination of skills than either court that turned him away.',
        },
        reception,
      };
    }
    ...
    if (st.obedience <= 2 || rf < 0.5) {
      return {
        ...
        evidence: {
          source: 'Dee\u2019s private diary, 1590\u20131608',
          text:
            'This is close to what actually happened after 1589. The spiritual diaries thin ' +
            'out and stop; the private diary continues for another twenty years in the same ' +
            'neat dated columns, recording weather, debts, illnesses and the occasional ' +
            'fruitless session with a poor skryer. Dee kept setting the stone out. Nothing ' +
            'much came. The game treats that as an ending rather than a failure state because ' +
            'the record does: there is no final scene, no renunciation, and no moment at which ' +
            'he decides it was not true.',
        },
        reception,
      };
    }
    ...
    if (f.covenant === 'delivered' && ro >= 0.9) {
      return {
        ...
        evidence: {
          source: 'True & Faithful Relation, 3 May 1587; Dee\u2019s private diary',
          text:
            'The covenant was signed by four people and is in Dee\u2019s hand, including his ' +
            'wife\u2019s consent \u2014 which is not the same as having her account of it, and no ' +
            'account of hers survives. What followed is documented: the partnership did not ' +
            'recover, Kelley stayed in Bohemia and was knighted, then imprisoned when he could ' +
            'not produce the gold he had promised, and died in 1597 after a fall. Jane Dee ' +
            'died of plague at Manchester in 1605. The game does not decide whether Kelley ' +
            'engineered the instruction, because nothing in the record decides it.',
        },
        reception,
      };
    }
    ...
    if (q.millennium >= 9 && f.year === 'affirmed') {
      return {
        ...
        evidence: {
          source: 'The Regiomontanus prophecy; Dee\u2019s own account of the burning',
          text:
            '1588 was loaded long before Dee said anything. A verse prophecy attributed to ' +
            'Regiomontanus had pointed at it for a century as the year of wonders or the end, ' +
            'so apocalyptic expectation was ambient across confessional lines. When it produced ' +
            'a scattered fleet and an ordinary harvest, the reckoning fell on everyone who had ' +
            'endorsed it. Dee burned years of the record himself afterwards, by his own ' +
            'account, which is a large part of why the archive survives only as a fragment. For ' +
            'a chronologer who had advised the Crown on calendar reform, a dated prophecy that ' +
            'fails is the one error the method cannot absorb.',
        },
        reception,
      };
    }
    ...
    if (f.print === 'allowed' && rn >= 0.95 && rf >= 0.9) {
      return {
        ...
        evidence: {
          source: 'M\u00e9ric Casaubon, A True & Faithful Relation (London, 1659)',
          text:
            'Casaubon printed the conversations in folio to discredit them \u2014 as a warning ' +
            'about where learned credulity leads \u2014 and thereby guaranteed their survival. ' +
            'Almost everything the modern reader knows about the actions comes through a book ' +
            'published by a man trying to bury them. His frontispiece pairs Dee with the ' +
            'Prophet Muhammad as fellow conversers with angels, intended as the worst thing he ' +
            'could say. And the subtitle states the imperial objective more plainly than Dee ' +
            'was ever told it: the conversations tended, \u201chad it succeeded, to a general ' +
    ...
    if (imperial > english) {
      return {
        ...
        evidence: {
          source: 'True & Faithful Relation, 1584\u201386; the Rudolfine court',
          text:
            'Rudolf II employed more occultists than any prince in Europe and acted on the ' +
            'advice of almost none of them. His Prague was a real centre of natural philosophy ' +
            '\u2014 Tycho and Kepler both worked there \u2014 so the reception Dee got was not ' +
            'credulity but collecting: knowledge acquired, catalogued, left unacted upon. One ' +
            'detail the game leaves for this note. Dee told Rudolf to his face that if he would ' +
            'not listen, the Lord would put him out of his seat. In 1611 Rudolf was forced to ' +
            'cede his crowns to his brother Matthias, and he died the following year. Make of ' +
            'that what you like; the record simply contains both halves.',
        },
        reception,
      };
    }
    ...
    if (english >= imperial && q.crown_english > 0) {
      return {
        ...
        epilogue:
          ...
          'meant by it.',
        evidence: {
          source: 'Brytanici Imperii Limites (1576\u201378); the Mathematicall Praeface (1570)',
          text:
            'England took the parts of Dee it could use and declined the rest. He advised on ' +
            'calendar reform in 1583 \u2014 his scheme was mathematically sound and was not ' +
            'adopted, on confessional grounds \u2014 on navigation and the north-west passage, and ' +
            'on the legal argument for English title to North America, which he built out of ' +
            'Arthur and Madoc. He coined the phrase \u201cBrytish Impire.\u201d None of that ' +
            'required believing him about angels, and the Crown never had to. The Mathematicall ' +
            'Praeface to Euclid, written for craftsmen rather than universities, outlived every ' +
            'slander made against its author.',
        },
        reception,
      };
    }
...
function longWayRound(reception) {
  return {
    ...
    evidence: {
      source: 'The Golden Dawn (founded 1888); Ashmole\u2019s collection; Sloane MSS',
      text:
        'The transmission really did run this way, and by a thoroughly undignified route. ' +
        'Elias Ashmole gathered Dee manuscripts in the 1670s, including a portion rescued ' +
        'from a chest whose contents a householder\u2019s maid had partly used to line pie ' +
        'dishes; the papers ended in the Sloane and Ashmole collections; Casaubon\u2019s ' +
        'hostile 1659 edition supplied the rest. In 1888 the Hermetic Order of the Golden ' +
        'Dawn reconstituted the whole system from those sources and worked the Watchtowers ' +
        'in earnest, which is how most people encounter Enochian magic today. Meanwhile the ' +
        'empire Dee named arrived without reference to the end of the world.',
    },
    reception,
  };
}
